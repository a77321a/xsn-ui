<template>
  <div class="x-tree-node-wrapper">
    <div
      v-for="(node, idx) in data"
      :key="'f_tree_node_' + idx"
      class="x-tree-node"
    >
      <div class="x-tree-node-content" @click.stop.prevent="_click(node)">
        <!-- icon -->
        <span
        ...
        <x-checkbox
          v-if="mutiple"
          :show-icon="
            !leaf || (leaf && (!node.children || node.children.length === 0))
          "
        ...
        <span
          ...
          :class="[
            leaf && node.children && node.children.length > 0 ? 'disabled' : '',
            selection === node.value ? 'text-' + activeColor : ''
          ]"
  ...
  methods: {
    ...
    _select(node) {
      ...
      if (!this.mutiple) {
        if (
          !this.leaf ||
          (this.leaf && (!node.children || node.children.length === 0))
        ) {
